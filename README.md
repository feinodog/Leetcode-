...
844 栈！！ 234 用辅助数组!!  或者快慢指针找到中点 然后翻转链表
139 动态规划 dpj可以 后面i-j再hash里 dpi可以
690 dfs bfs  468字符串  注意strtok 连续分割符是一个处理 
面试题55 - II. 平衡二叉树  递归 经典、、、 先递归求深度  先看左右的深度差值 然后判断左右是不是也是平衡 110一样的
执行用时 : 8 ms , 在所有 C 提交中击败了 95.16% 的用户 内存消耗 : 8.5 MB , 在所有 C 提交中击 看这个。。。
孤独像素 构造数组 判断  hash  先找点 然后判断行哈希
19 快慢指针 块的先动n 然后遍历 slow干掉一个
面试题 02.05. 链表求和
215 手撕qsort
53 南。。 分治 三段!!!!!  621!!!!!
